...
# --- ASIA ---
def transform_payments_asia(df: DataFrame, exchange_rates: dict) -> DataFrame:
    ...
    return df.select(
        ...
        col("method").alias("payment_method"),
        col("status").alias("payment_status"),
        (col("amount_jpy") * lit(rate)).alias("payment_amount_usd"),
        safe_to_timestamp_multi_formats(col("payment_time")).alias("payment_timestamp"),
        col("transaction_ref").alias("transaction_id"),
        col("gateway_resp").alias("gateway_response"),
        safe_to_timestamp_multi_formats(col("created")).alias("created_at"),
...
# --- EU ---
def transform_payments_eu(df: DataFrame, exchange_rates: dict) -> DataFrame:
    ...
    return df.select(
        ...
        col("payment_method"),
        col("payment_status"),
        (col("payment_amount_eur") * lit(rate)).alias("payment_amount_usd"),
        safe_to_timestamp_multi_formats(col("payment_timestamp")).alias("payment_timestamp"),
        col("transaction_id"),
        col("gateway_response"),
        col("psd2_compliant"),
...
def transform_payments_us(df: DataFrame,exchange_rates: dict) -> DataFrame:
    return df.select(
        ...
        col("payment_status"),
        col("payment_amount_usd"),
        safe_to_timestamp_multi_formats(col("payment_date")).alias("payment_timestamp"),
        col("transaction_id"),
        col("gateway_response"),
        safe_to_timestamp_multi_formats(col("created_at")).alias("created_at"),
        col("_region"),
...
def transform_payments(df: DataFrame, region: str, exchange_rates: dict) -> DataFrame:
    if region == "asia":
        return transform_payments_asia(df, exchange_rates)
    elif region == "eu":
        return transform_payments_eu(df, exchange_rates)
    elif region == "us":
        return transform_payments_us(df, exchange_rates)
    else:
        raise ValueError(f"Unsupported region: {region}")
