...
# @dag(
#     dag_id="transform_orders_csv",
#     default_args=DEFAULT_ARGS,
#     schedule_interval="@daily",
#     tags=["spark", "transformation", "csv"],
# )
# def transform_order_dag():

#     @task
#     def run_order_transform(region: str, load_date: str):
...
#         df_transformed = transform_order(df_raw, region)
...
#         run_order_transform(region=region, load_date=load_date)

# transform_order_dag = transform_order_dag()
...
from scripts.pyspark_jobs.orders import transform_orders_table
...
def transform_orders_dag():
    ...
    def run_transformation_asia():
        transform_orders_table(region="asia", table=TABLE_NAME, load_date="2025-06-08")

    @task
    def run_transformation_eu():
        transform_orders_table(region="eu", table=TABLE_NAME, load_date="2025-06-08")

    @task
    def run_transformation_us():
        transform_orders_table(region="us", table=TABLE_NAME, load_date="2025-06-08")
