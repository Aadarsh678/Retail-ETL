...
# @dag(
#     dag_id="transform_payments_csv",
#     default_args=DEFAULT_ARGS,
#     schedule_interval="@daily",
#     tags=["spark", "transformation", "csv"],
# )
# def transform_payments_dag():

#     @task
#     def run_payments_transform(region: str, load_date: str):
...
#         df_transformed = transform_payments(df_raw, region)
...
#         run_payments_transform(region=region, load_date=load_date)

# transform_payments_dag = transform_payments_dag()
...
from scripts.pyspark_jobs.payments import transform_payments_table

TABLE_NAME = "payments"


@dag(
    dag_id="transform_payments",
...
def transform_payments_dag():
    ...
    def run_transformation_asia():
        transform_payments_table(
            region="asia", table=TABLE_NAME, load_date="2025-06-08"
        )

    @task
    def run_transformation_eu():
        transform_payments_table(region="eu", table=TABLE_NAME, load_date="2025-06-08")

    @task
    def run_transformation_us():
        transform_payments_table(region="us", table=TABLE_NAME, load_date="2025-06-08")
