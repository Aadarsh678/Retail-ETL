...
# @dag(
#     dag_id="transform_shipments_csv",
#     default_args=DEFAULT_ARGS,
#     schedule_interval="@daily",
#     tags=["spark", "transformation", "csv"],
# )
# def transform_shipments_dag():

#     @task
#     def run_shipments_transform(region: str, load_date: str):
...
#         df_transformed = transform_shipments(df_raw, region)
...
#         run_shipments_transform(region=region, load_date=load_date)

# transform_shipments_dag = transform_shipments_dag()
...
from scripts.pyspark_jobs.shipments import transform_shipments_table

TABLE_NAME = "shipments"


@dag(
    dag_id="transform_shipments",
...
def transform_shipments_dag():
    ...
    def run_transformation_asia():
        transform_shipments_table(
            region="asia", table=TABLE_NAME, load_date="2025-06-08"
        )

    @task
    def run_transformation_eu():
        transform_shipments_table(region="eu", table=TABLE_NAME, load_date="2025-06-08")

    @task
    def run_transformation_us():
        transform_shipments_table(region="us", table=TABLE_NAME, load_date="2025-06-08")
