...
def normalize_weight(region, weight_col):
    return {
        "asia": col(weight_col) / lit(1000),
        "eu": col(weight_col),
        "us": col(weight_col) * lit(0.453592)
...
def parse_dimensions_inches(dim_col):
    ...
    height = regexp_extract(dim_col, r"[xX]\s*[0-9.]+\s*[xX]\s*([0-9.]+)", 1).cast("double")
...
def transform_asia_products(df):
    ...
    height = regexp_extract(col("size_info"), r"x\s*[0-9.]+mm\s*x\s*([0-9.]+)mm", 1).cast("double") / 10
