...
def safe_to_timestamp_multi_formats(date_col):
    clean_col = trim(
        regexp_replace(
            regexp_replace(date_col, "(?i)(st|nd|rd|th)", ""),  
            " +", " "  
        )
    )
...
# --- ASIA ---
def transform_cart_items_asia(df):
    return df.select(
        col("cart_item_id"),
        col("cart_id"),
        col("variant_id"),
...
# --- EU ---
def transform_cart_items_eu(df):
    return df.select(
        col("cart_item_id"),
        col("cart_id"),
        col("variant_id"),
...
def transform_cart_items(df: DataFrame, region: str) -> DataFrame:
    if region == "asia":
        return transform_cart_items_asia(df)
    elif region == "eu":
        return transform_cart_items_eu(df)
    elif region == "us":
