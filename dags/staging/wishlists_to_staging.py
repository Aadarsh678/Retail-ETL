...
# @dag(
#     dag_id="transform_wishlists_csv",
#     default_args=DEFAULT_ARGS,
#     schedule_interval="@daily",
#     tags=["spark", "transformation", "csv"],
# )
# def transform_whishlist_dag():

#     @task
#     def run_whishlist_transform(region: str, load_date: str):
...
#         df_transformed = transform_whishlist(df_raw, region)
...
#         run_whishlist_transform(region=region, load_date=load_date)

# transform_whishlist_dag = transform_whishlist_dag()
...
from scripts.pyspark_jobs.whishlists import transform_wishlists_table

TABLE_NAME = "whishlists"


@dag(
    dag_id="transform_wishlists",
...
def transform_whishlists_dag():

    @task
    def run_transformation_asia():
        transform_wishlists_table(
            region="asia", table=TABLE_NAME, load_date="2025-06-08"
        )

    @task
    def run_transformation_eu():
        transform_wishlists_table(region="eu", table=TABLE_NAME, load_date="2025-06-08")

    @task
    def run_transformation_us():
        transform_wishlists_table(region="us", table=TABLE_NAME, load_date="2025-06-08")
