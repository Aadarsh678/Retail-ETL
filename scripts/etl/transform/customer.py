...
def mask_phone(phone):
    if not phone:
        return None
    match = re.match(r"^(\+\d{1,3})(.*?)(\d{4})$", phone.strip().replace(" ", "").replace(".", ""))
    if match:
...
def transform_asia(df):
    return df.select(
        ...
        when(validate_email(trim(lower(col("email")))), trim(lower(col("email")))).otherwise(None).alias("email"),
...
def transform_eu(df):
    is_active = when(col("status_code") == 1, True).when(col("status_code").isin(0, 2, 3), False).otherwise(None)
    ...
    return df.select(
        ...
        col("country_code"),
        safe_to_timestamp_multi_formats(col("registration_date")).alias("registration_date"),
        safe_to_timestamp_multi_formats(col("last_login_at")).alias("last_login"),
        is_active,
        col("customer_tier").alias("customer_segment"),
        safe_to_timestamp_multi_formats(col("gdpr_consent_date")).alias("gdpr_consent_date"),
        safe_to_timestamp_multi_formats(col("data_retention_until")).alias("data_retention_until"),
